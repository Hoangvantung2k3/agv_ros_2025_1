...
ExplorationPlanning::ExplorationPlanning(ros::NodeHandle& nh, ros::NodeHandle& nh_p)
    ...
    obstacle_threshold_ = 50.0;
    boundary_threshold_ = 3.0;
    initialize();
}
...
void ExplorationPlanning::calculateAvgCentroidDistance() {
    ...
    if (first_two_cen_.centroidone.cenindex.x != first_two_cen_.centroidtwo.cenindex.x ||
        ...
        cy = first_two_cen_.centroidtwo.positionY * global_map_.info.resolution + global_map_.info.origin.position.y;
...
float ExplorationPlanning::calculateWeightPathCost(float map_density, float avg_centroid_distance) {
    ...
    float density_factor = (1.0 - map_density);
    float distance_factor = avg_centroid_distance / 20.0;
...
bool ExplorationPlanning::initialize() {
    ...
    robot_position_marker_ = std::make_shared<visual::Marker>(nh_, "pointvisiual", "map");
    ...
    centroids_marker_ = std::make_shared<visual::Marker>(nh_, "centroids", "map");
    ...
    windows_marker_ = std::make_shared<visual::Marker>(nh_, "windows", "map");
    ...
    viewpoints_marker_ = std::make_shared<visual::Marker>(nh_, "viewpoints", "map");
    ...
    choose_centroids_marker_ = std::make_shared<visual::Marker>(nh_, "choosecentroids", "map");
    ...
    choose_viewpoints_marker_ = std::make_shared<visual::Marker>(nh_, "chooseviewpoints", "map");
        ...
        for (int j = 0; j < centroids_.size() + 1; j++) {
            if (j == 0)
                ...
                if (i == 0) {
                    ...
                    endC.y = CONTXY2DISC(centroids_[j - 1].centroidposition.y - global_map_.info.resolution);
                    int pathcostRC;
...
void ExplorationPlanning::globalMapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg) {
    if (!global_map_.data.empty() && global_map_.data.size() == msg->data.size()) {
        ...
        for (size_t i = 0; i < msg->data.size(); ++i) {
            if (global_map_.data[i] != msg->data[i]) {
                diff_count++;
            }
        }
...
bool ExplorationPlanning::assessUnknownRegion(Centroid* choosecentroid) {
    ...
    choosecentroid->viewpoint_fin.clear();
    for (int i = 0; i < choosecentroid->viewpoint_fir.size(); i++) {
        ...
        endV.y = CONTXY2DISC(choosecentroid->viewpoint_fir[i].position.y - global_map_.info.resolution);
        int pathcostRV;
        std::vector<utils_ns::PointInt> tmppath;
        ...
        choosecentroid->viewpoint_fir[i].traver_degree = global_map_.data[choosecentroid->viewpoint_fir[i].indexX + choosecentroid->viewpoint_fir[i].indexY * width_];
        choosecentroid->viewpoint_fir[i].disgridnum = PassGrids.size();
    ...
    for (int j = 0; j < choosecentroid->viewpoint_fin.size(); j++) {
        float s_1 = max_rvpathcost > 0 ? float(choosecentroid->viewpoint_fin[j].rv_path_cost / max_rvpathcost) : 0;
        float s_2 = max_traver_degree > 0 ? float(choosecentroid->viewpoint_fin[j].traver_degree / max_traver_degree) : 0;
        float s_3 = max_disgridnum > 0 ? float(choosecentroid->viewpoint_fin[j].disgridnum / max_disgridnum) : 0;
        choosecentroid->viewpoint_fin[j].score = weight_path_cost_ * s_1 + weight_traver_degree_ * s_2 + weight_disgridnum_ * s_3;
        choosecentroid->evaluated_viewpoints.insert(std::make_pair(choosecentroid->viewpoint_fin[j].score, &choosecentroid->viewpoint_fin[j]));
    }

    if (choosecentroid->viewpoint_fin.empty()) {
        ROS_WARN("No valid viewpoints found for centroid at position (%f, %f)",
                 choosecentroid->centroidposition.x, choosecentroid->centroidposition.y);
        return false;
    }
    return true;
}
...
void ExplorationPlanning::execute(const ros::TimerEvent&) {
    ...
    if (findCenIs_ && !centroids_.empty()) {
        ...
        float sum = weight_path_cost_ + weight_traver_degree_ + weight_disgridnum_;
        if (sum > 0) {
            weight_path_cost_ /= sum;
            weight_traver_degree_ /= sum;
            weight_disgridnum_ /= sum;
        } else {
            weight_path_cost_ = weight_traver_degree_ = weight_disgridnum_ = 1.0 / 3.0;
        }
    }
...
geometry_msgs::Point ExplorationPlanning::index2coord(const int& x, const int& y) {
    ...
    p.x = DISCXY2CONT(x, resolution_) + ori_x_;
    p.y = DISCXY2CONT(y, resolution_) + ori_y_;
    return p;
}

utils_ns::Index2 ExplorationPlanning::coord2index(const geometry_msgs::Point& p) {
    utils_ns::Index2 index;
    index.x = CONTXY2DISC(p.x - ori_x_, resolution_);
    index.y = CONTXY2DISC(p.y - ori_y_, resolution_);
    return index;
}
} // namespace lrae_planner_ns